...
# 1. server.py의 위치(4deploy)에서 부모 폴더(mcp-agent-materials)로 올라간 뒤, 2mcp/.env로 들어갑니다.
current_dir = Path(__file__).resolve().parent  # -> 4deploy 폴더
parent_dir = current_dir.parent                # -> mcp-agent-materials 폴ced 폴더
env_path = parent_dir / "2mcp" / ".env"         # -> 2mcp/.env 최종 조준

# 2. 파이썬에게 실물 파일이 있는지 체크하게 하고, 기존 캐시를 override=True로 강제 덮어씁니다.
print(f"[DEBUG] 파이썬이 계산한 .env 조준 경로: {env_path}")
print(f"[DEBUG] 실물 파일이 이 경로에 진짜 존재하나요?: {env_path.exists()}")

load_dotenv(dotenv_path=env_path, override=True)

YOUTUBE_API_KEY = os.getenv("YOUTUBE_API_KEY") 
# 잘 들어왔는지 눈으로 확인하기 위한 디버그 코드 (선택 사항)
print(f"[DEBUG] 최종 매핑된 API Key: {YOUTUBE_API_KEY}")
...
async def get_youtube_comments(video_url: str, order: str = "relevance", max_results: int = 10) -> list:
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            print(f"Search Error: {e}", file=sys.stderr)
            # 빈 값 대신 실제 에러 원인을 프롬프트 창에 심어줍니다.
            return [{
                "ERROR_DETECTED": True,
                "error_message": str(e),
                "tip": "이 메시지가 보인다면 API Key 설정이나 구글 클라우드 플랫폼 설정을 확인해야 합니다."
            }]
            
            import os

# API 호출 직전이나 서버 시작 부분에 추가해서 터미널 로그를 확인해보세요!
raw_key = os.getenv("YOUTUBE_API_KEY")
print(f"[DEBUG] Loaded Key: {raw_key}")